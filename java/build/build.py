...
def build(args):
    ...
    if args.target_lib:
        ...
        output = subprocess.Popen(
            ["javac", "-d", DIST_DIR, "-source", "8", "-target", "8", "*.java"],
            cwd=INTEROPLIB_DIR,
            stdout=PIPE,
        )
        log_output(output)
        ...
        output = subprocess.Popen(
            ["jar", "cf",  LIBRARY_LOADER_JAR, "cangjie/lang/LibraryLoader.class", "cangjie/lang/internal/$$NativeConstructorMarker.class", "cangjie/interop/java/*.class"],
            cwd=DIST_DIR,
            stdout=PIPE,
        )
        log_output(output)
